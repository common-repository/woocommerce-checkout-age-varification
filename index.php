<?php 
/* 
Plugin Name:  Checkout Age Varification
Description: This plugin is used to varify age of user at  checkout. 
Version: 1.2
Author: Parth Shah
Author URI: www.parthcreation.in
Plugin URI: http://parthcreation.in/blog/2015/05/27/checkout-age-varification/
...
$checkout = WC()->checkout();
    ...
    echo '<div id="my_custom_checkout_field"><h3>' . __('Age Varification') . '</h3>';
...
function wcav_my_custom_checkout_field_process() {
	...
	$age=sanitize_text_field($_POST['my_field_name']);
	$y = explode("-", $age);
...
// get diffrence with cureent date
 ...
 if($year_diff == $order_number_start )
 {
    ...
    if ( ! sanitize_text_field($_POST['my_field_name']) || $final_age < $order_number_start ||  $final_age == $order_number_start)
